...
function ModalSigninComponent({showModal, setShowModal}) {
    ...
    return (<div className={`fixed z-10 inset-0 overflow-y-auto`} aria-labelledby={`modal-title`}
        ...
        <div
            className={` modal-dialog flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0`}>
            ...
            <div
                ...
                <button
                    type={`button`}
                    className={`absolute top-0 right-0 mt-3 mr-3 inline-flex justify-center rounded-b rounded-t border-none  px-4 py-2
                ...
                <div className={`bg-white px-4 pt-9 pb-4 sm:p-6 sm:pb-4 mt-6`}>
                    ...
                    <button
                        className={`w-full h-12 rounded-b rounded-t mt-6 focus:outline-none text-md text-[#b7bbbf] font-bold ${inputValue ? 'bg-red-500 text-[#ffffff]' : 'bg-[#f2f3f4] text-[#b7bbbf]'}`}>
                                     <span className="flex items-center justify-center">
                                      Tiếp tục
                                      <svg width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"
                                           version="1.1" xmlnsXlink="http://www.w3.org/1999/xlink"
                                           className={`d7ed-SwZDZ2 _9fe3-Ms4Hju ml-2 ${inputValue ? 'fill-current' : ''}`}>
